...
#else
inline std::string Utf8ToNative(absl::string_view input) {
  return std::string{input.data(), input.size()};
}

inline const std::string& NativeToUtf8(const std::string& input) {
  return input;
}

inline std::string NativeToUtf8(const char* input, size_t length) {
